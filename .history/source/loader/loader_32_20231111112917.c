...
static uint32_t reload_elf_file(uint8_t *file_buffer)
{
    ...
    for (int i = 0; i < elf_hdr->e_phnum; i++)
    {
        ...
        for (int k = 0; k < phdr->p_memsz - phdr->p_filesz; k++)
        {
            *dest++ = 0;
        }
    }
    return elf_hdr->e_entry; // 程序入口地址
}

static void dile(uint32_t error_code)
{
    for (;;)
}
void load_kernel(void)
{
    ...
    uint32_t kernel_entry = ((uint8_t *)SYS_KERNEL_LOAD_ADDR); // 解析elf文件进入内核
    // 如果内核入口地址是0则让程序死机
    if (kernel_entry == 0)
    {
        dile(-1);
