...
void irq_init(void)
{
    ...
    // 将异常处理配置到idt表中
    // 除0异常
    irq_install(IRQ0_DE, (irq_handler_t)exception_handle_divider);
    ...
    irq_install(IRQ5_BR, exception_handle_bound_range);
    irq_install(IRQ6_UD, exception_handle_invalid_opcode);
    irq_install(IRQ7_NM, exception_handle_device_unavailable);
    irq_install(IRQ8_DF, exception_handle_double_fault);
    irq_install(IRQ10_TS, exception_handle_invalid_tss);
    irq_install(IRQ11_NP, exception_handle_segment_not_present);
    irq_install(IRQ12_SS, exception_handle_stack_segment_fault);
    irq_install(IRQ13_GP, exception_handle_general_protection);
    irq_install(IRQ14_PF, exception_handle_page_fault);
    irq_install(IRQ16_MF, exception_handle_fpu_error);
    irq_install(IRQ17_AC, exception_handle_alignment_check);
    irq_install(IRQ18_MC, exception_handle_machine_check);
    irq_install(IRQ19_XM, exception_handle_smd_exception);
    irq_install(IRQ20_VE, exception_handle_virtual_exception);
